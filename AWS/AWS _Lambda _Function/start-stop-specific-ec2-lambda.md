...
# Create the role
aws iam create-role \
  --role-name ec2-start-stop-lambda-role \
  --assume-role-policy-document file://trust-policy.json

aws iam put-role-policy \
  --role-name ec2-start-stop-lambda-role \
  --policy-name ec2-start-stop-policy \
...
aws lambda create-function \
  ...
  --runtime python3.12 \
  --role arn:aws:iam::<ACCOUNT_ID>:role/ec2-start-stop-lambda-role \
  --handler lambda_function.lambda_handler \
  --zip-file fileb://function.zip \
  --timeout 15 \
  --environment "Variables={INSTANCE_ID=i-0123456789abcdef0}"
...
aws events put-targets \
  --rule ec2-start-stop-start-rule \
  --targets "Id"="1","Arn"="arn:aws:lambda:ap-south-1:<ACCOUNT_ID>:function:ec2-start-stop","Input"='{"action":"start"}'

aws events put-targets \
  --rule ec2-start-stop-stop-rule \
  --targets "Id"="1","Arn"="arn:aws:lambda:ap-south-1:<ACCOUNT_ID>:function:ec2-start-stop","Input"='{"action":"stop"}'

# Required manually via CLI (console does this automatically)
aws lambda add-permission \
  --function-name ec2-start-stop \
  --statement-id AllowEventBridgeStart \
  --action lambda:InvokeFunction \
  --principal events.amazonaws.com \
  --source-arn arn:aws:events:ap-south-1:<ACCOUNT_ID>:rule/ec2-start-stop-start-rule
...
aws lambda add-permission \
  ...
  --action lambda:InvokeFunction \
  --principal events.amazonaws.com \
  --source-arn arn:aws:events:ap-south-1:<ACCOUNT_ID>:rule/ec2-start-stop-stop-rule
...
variable "instance_id" {
  description = "The specific EC2 instance to schedule"
  type        = string
}
...
resource "aws_cloudwatch_event_rule" "start_rule" {
  name                = "ec2-start-stop-start-rule"
  schedule_expression = "cron(30 3 ? * MON-FRI *)" # 09:00 AM IST
}

resource "aws_cloudwatch_event_rule" "stop_rule" {
  name                = "ec2-start-stop-stop-rule"
  schedule_expression = "cron(30 14 ? * MON-FRI *)" # 08:00 PM IST
}
