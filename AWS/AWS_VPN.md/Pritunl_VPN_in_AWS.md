...
3. Protocol: **UDP**. Port: **1194** (matching the security group rule from Step 3 — change both together if you pick a different port).
4. Network: leave the default private range Pritunl suggests (e.g. `192.168.0.0/24`) — this is the internal IP range handed out to connecting clients, separate from your VPC's own CIDR.
5. Save.
